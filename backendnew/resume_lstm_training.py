...
def get_trained_tickers():
    """Get list of tickers that already have trained models"""
    trained_tickers = set()
    if os.path.exists(models_dir):
        for file in os.listdir(models_dir):
            if file.endswith('_lstm_model.h5'):
                ticker = file.replace('_lstm_model.h5', '')
                trained_tickers.add(ticker)
...
# Get already trained tickers
trained_tickers = get_trained_tickers()
print(f"Found {len(trained_tickers)} already trained tickers")

# Filter out already trained tickers
remaining_tickers = [ticker for ticker in all_tickers if ticker not in trained_tickers]
print(f"Remaining tickers to train: {len(remaining_tickers)}")

# Limit to first 20 remaining tickers for testing (remove this line to train on all remaining)
remaining_tickers = remaining_tickers[:20]

print(f"Will train on first {len(remaining_tickers)} remaining tickers:")
