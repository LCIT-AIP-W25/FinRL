...
Open-Source:
It’s a Python-based framework built on top of libraries like OpenAI Gym, TensorFlow, PyTorch, and Stable-Baselines3.
Pre-built Environments:
It includes ready-to-use financial environments modeled after OpenAI Gym environments, tailored for different trading tasks.
Datasets: 
FinRL integrates with real-world market data providers like Yahoo Finance, Quandl, Alpaca, and others to fetch stock, ETF, and cryptocurrency data.
Modular Design:
Users can easily define their custom strategies, policies, and trading objectives.


Key Components in FinRL:
Preprocessing:
Data is cleaned, normalized, and transformed into a format suitable for RL models.
Feature Engineering: 
Technical indicators, moving averages, and other derived features are added.
Model Training: 
Uses DRL algorithms like DQN, PPO, A2C, or TD3 to train the agent.
Backtesting: 
Tests the trained model on unseen historical data to evaluate performance.
...
Observation:
Market data like stock prices, indicators.
Actions:
